...
Обычная смена пароля — в «Профиль» (нужен текущий пароль). Администратор
может сбросить пароль любому пользователю через «Админка → Пользователи →
Сбросить пароль».

Если войти не может вообще никто (например, единственный Администратор
забыл пароль) — аварийный сброс `POST /auth/emergency-reset`, не требует
входа, а вместо этого статичный секрет `EMERGENCY_RESET_TOKEN` из `.env`
(по умолчанию пусто — эндпоинт выключен и всегда отвечает 404):

```bash
curl -X POST "$API/auth/emergency-reset" \
  -H "X-Reset-Token: $EMERGENCY_RESET_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"email":"admin@example.com","new_password":"НовыйПароль123"}'
```
